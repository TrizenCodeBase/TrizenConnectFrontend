...
export const fetchComments = async ({ skip = 0, blog_id, setParentCommentCountFun, comment_array = null }) => {
    ...
    await axios.post(config.serverDomain + "/get-blog-comments", { blog_id, skip })
        .then(({ data }) => {
...
const CommentsContainer = ({ blog, setBlog, commentsWrapper, setCommentsWrapper }) => {
    ...
    const loadMoreComments = async () => {
        try {
            let newCommentsData = await fetchComments({ 
                skip: commentsArr.length, 
                blog_id: _id, 
                setParentCommentCountFun: () => { },
                comment_array: commentsArr
            });

            setBlog({ ...blog, comments: newCommentsData });
        } catch (error) {
    ...
    return (
        ...
        <div className={"max-sm:w-full fixed " + (commentsWrapper ? "top-0 sm:right-0" : "top-[100%] sm:right-[-100%]") + " duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden"}>
            ...
            <CommentField action="comment" blog={blog} setBlog={setBlog} />
            ...
            {
                total_parent_comments > commentsArr.length ?
                    <button
